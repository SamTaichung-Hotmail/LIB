...
namespace AutoLib {
	...
	public:
		...
		AUTOMAUI_API int AddView(int nRow, int nCol, int NewViewID, CRuntimeClass * pViewClass,
			CCreateContext* pContext, int sx = 10, int sy = 10, bool bResourceInDll=false);
	...
	public:
		...
		AUTOMAUI_API void OnMachineMessage(MMessage *pMsg);
		AUTOMAUI_API void OnButtonKeyEvent(MachineBase::ButtonKeyIndex key, MButtonEvent::ButtonEvenCode code);
